import type { Listing, PropertyType } from './models';
...
export function mapApiListing(value: unknown, hostFallbackName = 'Host'): Listing {
  ...
  const validTypes: readonly PropertyType[] = [
    'EntireProperty',
    'Apartment',
    'House',
    'Duplex',
  ...
  const type = validTypes.includes(rawType as PropertyType) ? (rawType as PropertyType) : 'Apartment';
  ...
  return {
    ...
    status:
      data.status === 'PENDING' || data.status === 'APPROVED' || data.status === 'REJECTED'
        ...
        : undefined,
    occupancyStatus:
      data.occupancyStatus === 'Occupied' || data.occupancyStatus === 'Vacant' || data.occupancyStatus === 'Draft'
    ...
    availabilityNote: readString(data.availabilityNote, 'Availability depends on selected dates.'),
  };
}
